...
def main() -> None:
    ...
    summary += "- Candidate post-baseline source changes are enumerated in `implementation_evidence/phase-0/candidate-post-baseline-delta.json`; they are controlled implementation diff, not baseline drift.\n"
    ...
    summary += "- Frozen V2 named runtime preservation hashes are recorded in `implementation_evidence/phase-0/frozen-v2-preservation.json`; the full frozen history was intentionally not traversed.\n"
    summary += "- HOST full-tree preservation digest is recorded in `implementation_evidence/phase-0/host-preservation-manifest.json`; HOST is not a Git checkout.\n"
    ...
    summary += "- PowerShell: `$env:PYTEST_DISABLE_PLUGIN_AUTOLOAD='1'; python -m pytest -q`: **427 passed, 2 failed**; raw stdout is saved in `implementation_evidence/phase-0/baseline-pytest.stdout.txt`.\n"
