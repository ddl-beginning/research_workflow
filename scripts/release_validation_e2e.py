...
def run_installation_validation(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        relocated_client.call("workflow_start", {"workspace": str(relocated_project), "mode": "USER_CONFIRMED_BRIEF", "rough_requirement": "Relocation smoke project", "brief": {"goal": "Verify relocated Product startup and resume", "problem_statement": "Relocation smoke", "desired_outcome": "A resumable initialized project", "input": "A new project directory", "expected_output": "An approved project brief and resumable runtime", "scope": ["src"], "non_goals": ["provider execution", "legacy cleanup"], "constraints": ["preserve project identity", "do not import old state"], "available_assets": [], "acceptance_criteria": ["doctor ready", "resume preserves project identity"], "preferences": []}})
